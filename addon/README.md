...
- **RESTful API**：提供配置、状态、任务、日志、定时任务、UDPXY 和回放代理接口
...
- `GET /api/v1/interfaces/{name}` - 获取指定网络接口详情
- `POST /api/v1/jobs/{job_type}` - 手动触发任务，`job_type` 支持 `m3u`、`epg`、`logos`
- `GET /api/v1/logs` - 获取日志
- `POST /api/v1/logs/clear` - 清理日志
- `GET /api/v1/cron` - 获取定时任务状态
- `POST /api/v1/cron` - 设置定时任务
- `DELETE /api/v1/cron` - 删除定时任务
- `GET /api/v1/udpxy` - 获取 UDPXY 状态
- `POST /api/v1/udpxy/actions` - 执行 UDPXY 操作，请求体示例：`{"action":"start"}`
- `GET /api/v1/udpxy/config` - 获取 UDPXY 配置
- `PUT /api/v1/udpxy/config` - 更新 UDPXY 配置
